# 004 模型层选 AI SDK v5 + providerOptions 逃生口
...
**Decision**：用 Vercel AI SDK v5 打底，但 **kernel 不直接 import AI SDK**——中间隔一层 `packages/model` 的窄接口（`generate(req): AsyncIterable<ModelEvent>`），并从第一天起把 `providerOptions` 与原始 usage 字段**透传到事件流**，不在适配层丢弃。
