...
- 日期：2026-09-17
- 状态：提议（随 `docs/prd/M8.md` 拍板生效；落实 `docs/adr/013` 押后的 Tailwind / shadcn 决定）
...
- **持久化**：主题与色板是「这台设备的偏好」，存浏览器 `localStorage`（读写包 try/catch，失败退回跟随系统 + 默认蓝），不进 daemon 配置。
...
Tailwind v3（v4 的 CSS-first 配置正好承接 CSS 变量 token，不需要 `tailwind.config.js`）；把主题存 daemon（远程连同一台 daemon 的不同设备往往想要不同外观）。
