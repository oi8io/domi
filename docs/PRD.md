...
> 覆盖 M0–M13 全部 14 个里程碑（M7 为 v1.8 追加，M8 为 v1.10 追加，M9 为 v1.12 追加，M10 为 v1.13 追加，M11 / M12 为 v1.14 补记与追加，M13 为 v1.17 追加）。
> **v1.18** · 2026-09-24（初版 v1.0 · 2026-09-14）· 作者：PM 环节
> 上位文档：`PRD-VISION.md` v1.1（不变量，冲突时以其为准）
> 变更：v1.0 经两轮独立门禁审计后修订，见 `docs/qa/prd-gate-audit-v1.0.md`
> **v1.18 回写**（2026-09-24，`docs/adr/014`）——用户拍板：**删除 TOML 配置支持**，结束 ADR-014 的过渡期（原定 M4 再批准门删，一直没删）。`~/.domi/config.toml` 与 `DOMI_CONFIG=*.toml` 不再读取（后者一律按 YAML 解析，TOML 语法直接报配置错误）；`domi init --from-toml` 删除；`domi doctor` 只在 `config.toml` 还躺在 `~/.domi` 里时提示「已不再读取，手动搬到 config.yaml」。没有 AC 改动——v1.4 已把四处 AC 的格式名改成 YAML，本次只删过渡分支。
> **v1.17 回写**（2026-09-24，`docs/prd/M13.md`）——**仅追加** M13 一章（2 条需求，用户拍板后直接 `COMMITTED`，缺复核记录）：运行中补充（排队、下一步送达）与中断当前轮。PRD-M11-007「TUI 会话中断」（占位、未建任务，等的「终止方案 S1–S4」在仓库里没有记录）由 PRD-M13-002 接手并扩为 Web + TUI，原条目保留、标注接手。`session.submit` 的忙时语义（PRD-M3-004 AC-3）不改——补充走新 RPC。
> **v1.16 回写**（2026-09-23，`docs/prd/M11.md` · `docs/adr/028`）——用户觉得 Markdown 渲染「显示上不够协调和清晰，跟整体主题不匹配」，拍板：按对齐原型的建议稿落地、做轻量语法高亮、TUI 一起调。PRD-M11-003 追加 AC-9（语法高亮，推翻分册「两端都不做代码块语法高亮」）与 AC-10（排版对齐原型）。
> **v1.15 回写**（2026-09-23，M12 第二轮拍板，`docs/prd/M12.md`）——PRD-M12-004 的 AC-2…AC-4 划掉，追加 AC-7…AC-10：问题框参考 Claude Code 的 AskUserQuestion；**计划必须**（任务强制、会话可选，计划常驻上下文，中断能接着做）；审批跟确认模式走；中断后一键续跑。PRD-M12-002 追加 AC-9（状态栏显示确认模式）。项目设置里的「计划审阅策略」（PRD-M8-005 AC-3 已在 v1.14 划掉）随之从界面移除。
...
| M13 · 运行中对话 | `COMMITTED` | 2026-09-24 追加（缺复核记录），见 `docs/prd/M13.md` |
...
| M13 | 运行中对话 | — | 3.5 天 | agent 跑着的时候，用户能不能补一句、能不能叫停 | —（v1.17 追加） |
...
- **AC**
  ...
  - AC-2：模型请求工具调用时执行并回灌结果继续；单轮达到 ~~20 次~~ `loop.maxToolCalls`（默认 100）次工具循环时强制停止，产生 `error{recoverable:true}` 并提示用户（v1.13 回写：护栏改为可配置，默认值定为 100，见 `docs/qa/M10-prd-review.md` B6）
...
- **AC**
  ...
  - AC-7：记忆管理：上下文策略（不处理 / 结构化清理 / 清理 + 自动压缩）、逐字保留轮数、压缩阈值、记忆抽取间隔，都经 `config.set` 保存；后两项压缩参数只在自动压缩时可编辑
...
# M10 · 会话体验与运行控制 `COMMITTED`

> **v1.13（2026-09-19）新增**。触发：用户提出五个需求点（标题接入运行时、运行时护栏可配置、TUI 设置、思考折叠、默认模型互斥确认）。
> 再批准门材料与拍板见 `docs/prd/M10.md`，复核见 `docs/qa/M10-prd-review.md`。

**要回答的问题**：会话列表可读、护栏可调、TUI 能切语言、思考不淹没对话。

**不做什么**：不改 `generateTitle()` 的机制（LLM 生成 + 降级首条输入前 40 字）· 不做 provider 级默认开关（默认保持模型级）·
TUI 本轮不做完整设置页（只做语言切换）· 不引入思考折叠的持久化与个性化 · 不新增模型微调 / 自研模型。

**预算**：3 天。超 20%（≈3.6 天）先砍 TUI 语言设置的热切换方案，退回「下次启动生效」。
...
- **层级**：Negotiable · **优先级**：P1（验证为主，无新代码）
...
**M10 DoD**：用户新建会话跑完第一轮，侧边栏出现可读标题；在设置页改过 loop 上限并在下一轮生效；TUI 里切一次语言重启后界面变化；长思考链在 Web 与 TUI 都默认折叠、可展开。→ 只有用户能做。

### PRD-M11-001 · 进入会话默认滚动到底
- **用户价值**：打开长会话直接停在最新内容，不用手动拖到底。
- **AC**：切会话/挂载时强制贴底一次；之后沿用距底阈值跟随。详见 `docs/prd/M11.md`。
- **层级**：Negotiable · **优先级**：P1

### PRD-M11-002 · 运行中自动贴底与新消息提示
- **用户价值**：流式输出自动贴底；用户上翻看历史时不被打断，底部浮条提示「N 条新消息」，点击回底。
...
- **AC**：Web assistant 分支 react-markdown+remark-gfm；TUI 复用 remark 解析器自写 mdast→ANSI（v1.14 补记：2026-09-21 复核扩围）；降级纯文本；script/javascript: 不渲染。详见分册。
  - AC-9：代码块语法高亮，Web + TUI 同一份分词与配色（Primer 语法色，对比度 ≥ 4.5:1）；不认识 / 没写语言不猜；Web 按需加载、不进首屏包（v1.16，ADR-028）
  - AC-10：排版对齐原型——标题小阶梯、代码块 / 表格与工具调用同一套卡片外观、引用同「思考」、列表记号用次要色；TUI 标题不带 #、代码块带语言头与左边线、表格只留横线并按显示宽度对齐（v1.16）
- **层级**：Negotiable · **优先级**：P1

### PRD-M11-004 · 展开后显示完整内容
- **用户价值**：tool 调用/结果展开看到完整参数与结果，不是省略号。
- **AC**：TranscriptItem 带 detail（不截断）；展开 `<pre>` 优先 detail。详见分册。
- **层级**：Negotiable · **优先级**：P1
...
- **用户价值**：打开任一前端就自动连后台，不用手动起 `pnpm domid`；连不上给可操作指引。
- **AC**：ensureDaemon 回归；Web 断连指引；desktop 壳 blocked（ADR-021 + 无 Rust）。详见分册。
- **层级**：Negotiable · **优先级**：P1

### PRD-M11-007 · TUI 会话中断
- **用户价值**：长任务在 TUI 里可被干净中断。
- **AC**：占位，等终止方案 S1–S4 拍板。详见分册。
- **层级**：Negotiable · **优先级**：P2
- **v1.17**：由 PRD-M13-002 接手（语义已定，扩为 Web + TUI），本条不再单独建任务。
...
- **AC**
  ...
  - AC-9：状态栏常驻显示当前确认模式（Web + TUI）；「全部放行」用警示色
...
- **AC**
  ...
  - ~~AC-2：需要确认时，确认弹框按问题数分 tab，每题 abcd… + 自定义输入；确认后按结果执行~~（v1.15 划掉，见 AC-7…AC-10）
  - ~~AC-3：无需确认时，用户信息无阻止提示 → 直接执行；有阻止提示 → 先问~~（v1.15 划掉，见 AC-7…AC-10）
  - ~~AC-4：新任务走新流程（替代 PRD-M8-005 AC-2 / AC-3），行为不倒退~~（v1.15 划掉，见 AC-7…AC-10）
  - AC-5：转长任务能力保留
  - AC-6：新确认弹框遵守会话确认模式（PRD-M12-002 AC-6）
  - AC-7：问题框（参考 Claude Code 的 AskUserQuestion）：模型用 `ask.user` 提问，一次 1–4 题；每题一个 tab（标签 ≤12 字），2–4 个选项（a–d，各带一句说明）+「其他」自定义输入，可声明多选；最后一个 tab 核对全部答案再提交。没人能回答 → 拒答，不替人选；不认识问题框的端（旧客户端、Telegram）按普通表单降级
  - AC-8：**计划必须**：任务会话里，动手（只读之外的工具）之前必须先用 `plan.update` 写出计划（步骤 + 状态），否则运行时拦下这次调用、让模型先写计划；自由会话可选。计划每次更新落 `plan.update` 事件，并常驻上下文（压缩也不丢），中途断了能照着接着做
  - AC-9：审批跟确认模式走：每次都问 → 计划必须批准才能动手；按需 → 只有用户消息里有阻止提示（先别动 / 先给方案 / 等我确认 / don't start…）才要批准，模型自己有疑问就用问题框问；全部放行 → 不审批。审批本身也是问题框（批准执行 / 批准并转成长任务 / 自定义写修改意见），结果落 `plan.decided`；没批准前只读之外的工具一律拦下
  - AC-10：中断续跑：计划还有没做完的步骤、会话空闲时，Web / TUI 显示「计划还剩 N 步 · 继续」，一键从没做完的步骤接着做；上一轮是被打断的（中断 / 出错 / 进程挂掉后补的一致点）时突出显示
- **验收方式**：权限引擎回归 + runtime 新流程测试 + Web / TUI 确认弹框组件测试
- **层级**：Negotiable · **优先级**：P1
- **现状（v1.15）**：AC-1 已做（v1.14）；AC-5 / AC-6 / AC-7…AC-10 由 TASK-M12-006…008 实现

**M12 DoD**：模型切换只有下拉；对话里切三档确认模式且 A / B 会话互不影响、状态栏看得到档位；新任务先写计划、有疑问弹多 tab 问题框、每次都问档下计划要批准；中途关掉再打开能一键续跑；改会话标题后两处同步。→ 只有用户能做。

# M13 · 运行中对话 `COMMITTED`

**要回答的问题**：agent 跑着的时候，用户能不能补一句（不打断，下一步生效）、能不能叫停。分册 `docs/prd/M13.md`（拍板记录、现状核查、不做什么）。

### PRD-M13-001 · 运行中补充（排队，下一步送达）
- **用户价值**：想起要补一句，不用等它跑完、也不用打断它——补充在下一步开始前送到。
- **AC**
  - AC-1：新事件 `user.note`（首版只收文本），协议只增不改
  - AC-2：补充只在安全点落盘（每步开头 / 收场前），任何 `tool.call` 与其 `tool.result` 之间不出现 `user.note`
  - AC-3：模型准备收场时队列非空 → 送达并再跑一步，照常受护栏约束
  - AC-4：`user.note` 不是轮边界（分页、引用本轮、分支、标题不受影响）
  - AC-5：排队看得见（排队条 + 条数，多端同步）；送达后以「补充」气泡出现在模型真正看到它的位置
  - AC-6：送达前可撤回；已送达不可撤回、不产生事件
  - AC-7：异常收场（出错 / 护栏 / 用量上限 / 中断）时未送达的补充退回发送端输入框草稿，不自动跑
  - AC-8：补充不是确认 / 回答，不改权限决策（INV-03）
  - AC-9：Web / TUI 运行中输入框不禁用，回车 = 补充；闲时行为不变
  - AC-10：评估回放时补充在录制位置送达（INV-13）
- **验收方式**：kernel loop / buildContext / paginate 单测 + daemon 并发与异常收场单测 + client-core store 单测 + Web / TUI 组件测试 + 协议快照
- **层级**：Negotiable · **优先级**：P0

### PRD-M13-002 · 中断当前轮（Web + TUI）
- **用户价值**：跑偏了能立刻叫停，已经做的不丢，停下后可以接着说。接手 PRD-M11-007。
- **AC**
  - AC-1：新 RPC 中断当前轮；会话闲时幂等、不产生事件
  - AC-2：模型输出中 → 停止，已生成内容照常落盘
  - AC-3：工具执行中 → 发中止信号；未完成 / 未开始的调用补 `tool.result{ok:false, reason:'interrupted'}`，事件流无悬空调用
  - AC-4：落本轮结束标记（stopReason `interrupted`，记录来自哪个端）；「计划还剩 N 步 · 继续」照常出现
  - AC-5：不自动续跑、不自动重试
  - AC-6：TUI `Esc` 中断（不退出，`Ctrl+C` 不变）；Web 运行中出现「停止」；中断后输入框可用
  - AC-7：中断时未送达的补充按 PRD-M13-001 AC-7 退回草稿
- **验收方式**：runtime 单测（流中断 / 工具中断 / 幂等 / 结束标记）+ recovery 一致点回归 + daemon 单测 + TUI 按键 / Web SSR 测试
- **层级**：Negotiable · **优先级**：P1

- **现状（2026-09-24）**：PRD-M13-001 / 002 由 TASK-M13-001…006 实现，`pnpm check` 各项全绿；手测归用户（DoD）。

**M13 DoD**：长任务跑着时补一句，下一步它就照着改、气泡出现在对的位置；排队中能撤回；两端都能叫停，停下后内容还在、能一键续跑或直接说新的话。→ 只有用户能做。
