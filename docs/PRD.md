...
| M4 · 有灵魂 | `SKETCH` | 轮廓级，进入前必须重写 |
| M5 · 会干活 | `SKETCH` | 轮廓级，进入前必须重写 |
...
| M2 | 可信 | v0.2 | **19 天** | 用户能否看懂并信任它做的每一步 | 轨迹截图 + 回放评估 |
| M3 | 三端 | v0.3 | 15 天 | 终端起的任务能否在浏览器里接着看 | 跨端演示 |
...
**不做什么**：不做 Web/桌面端（M2-005 AC-5 的 HTML 导出是**静态快照文件**，不是交互式客户端，二者边界见该条 AC）· 不做 Soul · 不做 Skill/插件 · 不做编排。

**预算**：15 天。

### PRD-M2-001 · MCP client（对齐 2026-07-28 规范）
...
- **AC**
  - AC-1：历史事件可按全文（FTS5）与语义（向量）检索，每条结果携带 `sessionId` + `seq` 可溯源
...
- **验收方式**：`bun test memory/episodic.spec.ts` + `bench/episodic-10w.ts`（合成数据性能测试，CI）
- **层级**：Negotiable · **优先级**：P1
