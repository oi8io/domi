...
| M7 · 会写代码 | `SKETCH` | 2026-09-16 追加，待再批准，见 `docs/prd/M7.md` |
...
> 本章是轮廓，**进入前要过再批准门**（材料见 `docs/prd/M7.md`）。
...
- **归类**（PRD-VISION §6）：搜索用 ripgrep（引用真库）；精确替换与过期写保护抄算法（Claude Code / Codex 的编辑工具）
- **层级**：Invariant（INV-03、INV-05）· **优先级**：P0

### PRD-M7-002 · 项目规矩文件与工作区信任
...
- **AC**
  ...
  - AC-3：**未被信任的工作区不加载规矩文件**。第一次在某个仓库根打开会话时询问是否信任，决定落 `workspace.trust` 事件；非交互模式默认不信任（断言：未信任仓库的 `AGENTS.md` 内容不出现在发给模型的请求里）
  - AC-4：规矩文件只能影响提示词，**不能声明钩子、权限规则或要执行的命令**；断言其中写的权限规则不改变任何权限决策
