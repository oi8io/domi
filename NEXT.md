在做: **M2 七条里已交付六条**（M2-008 回放、M2-002 清理、M2-005 轨迹、M2-007 迁移、M2-006 注入防护、M2-003 LLM 压缩），
      加上把「提测」需要的东西补齐 —— `pnpm check` 一条命令跑完全部：
      typecheck + **15 道守卫** + **519 个测试** + L1 轨迹回放；另有 `pnpm smoke` 跑单二进制冒烟。
      ...
      - **LLM 压缩**（PRD-M2-003）：保边压中 + 五字段结构化摘要；`/compact` 手动、到窗口 70% 自动。
        压缩只追加一条 `ctx.compact`，**原始事件逐条 sha256 不变**，所以 AC-5 的重放等价性是免费的。
...
卡在: 已不卡了。**MCP 两条（M2-001/M2-009）按你的决定移到 M3**（`docs/adr/011`），
      M2 预算 19 → 14 天、M3 15 → 20 天，合计不变，AC 原文一个字没改。
      M2 只剩 **M2-004（L2 跨会话检索）**；它要 FTS5 + 向量，向量那半要先有真实语料（`docs/adr/010`）。
