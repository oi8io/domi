在做: M2 里**不依赖再批准门**的五条已经做完（M2-008 回放、M2-002 清理、M2-005 轨迹、M2-007 迁移、M2-006 注入防护），
      加上把「提测」需要的东西补齐 —— `pnpm check` 一条命令跑完全部：
      typecheck + **15 道守卫** + **494 个测试** + L1 轨迹回放；另有 `pnpm smoke` 跑单二进制冒烟。
      ...
      - **注入防护**（PRD-M2-006）：十六条注入载荷，**每一条断言的都是权限层拒绝**，
        没有一条依赖模型是否被骗到。工具结果带私有区边界标记，伪造边界会被剥掉。
