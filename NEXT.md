在做: **M12「体验修订」COMMITTED**（2026-09-23 四条全部拍板定案）：
      ① 模型纯下拉（Web+TUI 都不手填，resolveModel 删）/ ② 三档会话级+对话框内选，UI「确认模式」配置键 permissions.mode、TUI /mode、全仓 review 清零 /
      ③ plan/act 取消=B方案（产出计划→确认弹框多tab abcd+自定义→执行；无确认点且无阻止提示直接开跑）/ ④ 会话标题改名同步+空标题口径统一首条40字。
      待 SPEC/任务拆分（任务编号从 TASK-M12-000 起）。
      **M11「会话体验深化」000–009 全部 done**（009 窗口化加载已完成：尾部窗口+seq 锚点分页，commit 3be34ab 等）。
