 ...
 *
 * auto：git 仓库里，且（工作区有未提交改动 或 多节点 或 定时触发）才隔离。工作区干净时直接改 + 步级快照就够安全，
 * 用户还能在自己的编辑器里实时看到改动。多节点在规划批准之后才知道：那时给 DAG 运行单独建工作区（shape = 'dag'）。
 */
...
export function decideIsolation(opts: {
  ...
  if (opts.shape === 'dag') return { isolate: true, reason: 'dag' }
